...
shinyML_classification <- function(data = data,y, share_app = FALSE,port = NULL){
  ...
  # Test if y class correspond to character or factor 
  if (!(eval(parse(text = paste0("class(data$",y,")"))) %in% c("character","factor"))){
    stop("y column class must be character or factor")
  }
    ...
    ui = dashboardPage(skin = textOutput("color_app"),
                       ...
                       dashboardSidebar(
                         sidebarMenu(
                           ...
                           radioButtons(label = "Framework",inputId = "framework", choices = c("H2O","Spark"),selected = "H2O",inline = T),
...
shinyML_classification(data =longley2,y = "GNP",share_app = FALSE)
